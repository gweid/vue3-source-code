...
Vue 3 的编译过程可以分为三个主要阶段：

1. **解析阶段（Parsing）**：将模板字符串解析成抽象语法树（AST）
2. **转换阶段（Transformation）**：对 AST 进行优化处理
3. **代码生成阶段（Code Generation）**：将优化后的 AST 转换为可执行的渲染函数



#### 解释阶段

在解析阶段，Vue 3 会将模板字符串转换为抽象语法树（AST）



#### 转换阶段

在转换阶段，Vue 3 会对 AST 进行一系列转换、优化处理：



  **转换：**
  - v-if/v-else/v-else-if → 条件渲染函数
  - v-for → 循环渲染函数
  - v-model → 双向绑定逻辑
  - @click 等事件 → 事件处理器
...
- **静态提升（Static Hoisting）**：将静态节点提取为常量，避免重复创建
- **PatchFlag 标记**：为动态节点添加数字标记，指示运行时需要比对的属性类型
- **树结构优化（Block Tree/Tree Flattening）**：将动态子节点存储为扁平数组，跳过静态中间层
- **静态属性提升**：将静态属性提取为常量，避免重复生成
- **事件侦听器缓存**：缓存内联事件处理器，避免重复创建函数



#### 生成阶段

在代码生成阶段，优化后的 AST 会被转换为可执行的 JavaScript 渲染（render 函数）函数
